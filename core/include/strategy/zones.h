...
// Параллельный массив типов границ для keeper_zone_points(): индекс совпадает
// со стартовой точкой сегмента (i → ребро points[i]→points[(i+1)%N]).
// BrakeType::Off означает «не применять Segment::apply на этом ребре»
// (виртуальная граница зоны, без физической стенки); Wall — бортик поля;
// VirtualNormal — разметка ворот. Оба getter'а проецируются из общего источника
...
// Параллельный массив типов границ для attacker_zone_points(). Внешний периметр
// поля помечен BrakeType::Wall, верхняя граница выреза зоны вратаря —
// BrakeType::VirtualLow (ослабленное торможение, чтобы нападающий не отставал от
// врага в погоне); разметка ворот и боковины выреза — BrakeType::VirtualNormal.
