...
void Strategy::run_test(Robot& robot, Object& ball, Object& goal) {
...
void Strategy::run_test_movement(Robot& robot) {
  ...
  auto handle = [&](Robot& robot, Vec& target) {
    robot.rotation = normalize_angle(point_rotation[state] - robot.field_angle);
    robot.rotation_limit = 15;
