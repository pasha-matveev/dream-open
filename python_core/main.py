...
front_goal = Goal(
    105,
    camera.yellow_goal if front_color == "yellow" else camera.blue_goal,
    front_color,
)
back_goal = Goal(
    -105,
    camera.blue_goal if front_color == "yellow" else camera.yellow_goal,
    "blue" if front_color == "yellow" else "yellow",
)

vis = Visualization(
    attacker_field, ball, robot, obstacles, front_goal, back_goal, fps=1
)
uart = UART()
lidar = Lidar(args)
...
try:
    ...
    while True:
        ...
        if lidar.new_data:  # interpret data from lidar
            ...
            if (
                abs(1 - lidar.field.width / 250) < 0.2
                and abs(1 - lidar.field.height / 186) < 0.2
            ):
        ...
        if robot.is_attacker:  # attacker ------------------------------------------------------------------------
            if (
                (
                    (not attacker_field.is_inside(ball.pos))
                    or ball.pos.y < -50
                    or (not kick_field.is_inside(robot.pos))
                )
                and robot.pos.dist(ball.pos) < 30
            ) and ball.pos.y > -60:
                straight_path = True

            if (
                attacker_field.is_inside(ball.pos) and robot.pos.dist(ball.pos) > 40
            ) or ball.pos.y < -65:
                straight_path = False
            ...
            if (time.time() - ball.visible_tm) < 3:
                if not robot.emitter:
                    ...
                    if time.time() - robot.first_tm < 0.5:
                        ...
                        curve_kick = (not kick_field.is_inside(robot.pos)) and (
                            math.cos(
                                robot.pos.angle(front_goal.center)
                                - (-robot.gyro + math.pi / 2)
                            )
                            < 0.85
                        )
                        ...
                        if not curve_kick:
                            ...
                            robot.rotation = angle
                            robot.vel = Vector.from_angle(
                                (-robot.gyro + math.pi / 2), 10
                            )
                            robot.rot_limit = 20
                            robot.dribbling = 40
                            ...
                            robot.dribbling = 100
                            if (
                                robot.pos.dist(kick_point) < 2
                                and math.cos(angle - (-robot.gyro + math.pi / 2)) > 0.98
                            ):
            ...
            if (time.time() - ball.visible_tm) < 1:
                ...
                if active_deff:
                    if robot.emitter:
                        ...
                        robot.vel = Vector.from_points(robot.pos, ball.pos)
                        robot.vel.length = (
                            robot.vel.length * 3 + 20 if robot.vel.length > 10 else 0
                        )
                        robot.dribbling = 60
                        robot.rotation = robot.pos.angle(ball.pos)
                        robot.rot_limit = 30
                ...
                close_obstacles = [
                    ...
                    if robot.pos.dist(obstacle.pos) < 70
                ]

                big_obstacle: None | Obstacle = None
                ...
                if time.time() < (big_obstacle_tm + 3):
                    assert big_obstacle
                    robot.vel = Vector.from_points(
                        robot.pos, Point(big_obstacle.pos.x, -85)
                    )
        ...
        if uart.writable:
            uart.write(
                "ffffii",
                -robot.vel.angle + math.pi / 2 + gyro_correction,
                robot.vel.length,
                -robot.rotation + math.pi / 2,
                robot.rot_limit,
                robot.dribbling,
